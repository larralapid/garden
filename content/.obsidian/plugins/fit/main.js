...
var FitPush = class {
  ...
  async pushChangedFilesToRemote(localUpdate, saveLocalStoreCallback, disableOpsNotif) {
...
var FitSettingTab = class extends import_obsidian3.PluginSettingTab {
  constructor(app, plugin) {
    ...
    this.githubUserInfoBlock = () => {
      ...
      new import_obsidian3.Setting(containerEl).setHeading().setName("GitHub user info").addButton((button) => button.setCta().setButtonText("Authenticate user").setDisabled(this.authenticating).onClick(async () => {
        if (this.authenticating)
          return;
        await this.handleUserFetch();
      }));
      this.ownerSetting = new import_obsidian3.Setting(containerEl).setDesc("Input your personal access token below to get authenticated. Create a GitHub account here if you don't have one yet.").addExtraButton((button) => button.setIcon("github").setTooltip("Sign up on github.com").onClick(async () => {
      ...
      this.patSetting = new import_obsidian3.Setting(containerEl).setName("Github personal access token").setDesc("Remember to give it access for reading and writing to the storage repo.").addText((text) => text.setPlaceholder("GitHub personal access token").setValue(this.plugin.settings.pat).onChange(async (value) => {
    ...
    this.repoInfoBlock = async () => {
      const { containerEl } = this;
      new import_obsidian3.Setting(containerEl).setHeading().setName("Repository info").setDesc("Refresh to retrieve the latest list of repos and branches.").addExtraButton((button) => button.setTooltip("Refresh repos and branches list").setDisabled(this.plugin.settings.owner === "").setIcon("refresh-cw").onClick(async () => {
        await this.refreshFields("repo(0)");
      }));
      new import_obsidian3.Setting(containerEl).setDesc("Select 'Add a README file' if creating a new repo. Make sure you are logged in to github on your browser.").addExtraButton((button) => button.setIcon("github").setTooltip("Create a new repository").onClick(() => {
        window.open(`https://github.com/new`, "_blank");
      }));
      this.repoSetting = new import_obsidian3.Setting(containerEl).setName("Github repository name").setDesc("Select a repo to sync your vault.").addDropdown((dropdown) => {
      ...
      this.branchSetting = new import_obsidian3.Setting(containerEl).setName("Branch name").setDesc("Select a repo above to view existing branches.").addDropdown((dropdown) => {
    ...
    this.viewLinkBlock = () => {
      ...
      new import_obsidian3.Setting(containerEl).setHeading().setName("Link");
      const linkDisplay = new import_obsidian3.Setting(containerEl).setName("View your vault on GitHub").setDesc(this.repoLink).addExtraButton(
    ...
    this.localConfigBlock = () => {
      const { containerEl } = this;
      new import_obsidian3.Setting(containerEl).setHeading().setName("Local configurations");
      new import_obsidian3.Setting(containerEl).setName("Device name").setDesc("Sign commit message with this device name.").addText((text) => text.setPlaceholder("Device name").setValue(this.plugin.settings.deviceName).onChange(async (value) => {
        this.plugin.settings.deviceName = value;
        await this.plugin.saveSettings();
      }));
      const checkIntervalSlider = new import_obsidian3.Setting(containerEl).setName("Remote check interval").setDesc(`Automatically check for remote changes in the background every ${this.plugin.settings.checkEveryXMinutes} minutes.`).addSlider(
...
var FitSync = class {
  ...
  async resolveFileConflict(clash, latestRemoteFileSha) {
    ...
    const localFileContent = (0, import_obsidian4.arrayBufferToBase64)(await this.fit.vaultOps.vault.readBinary(localFile));
...
var VaultOperations = class {
  ...
  async writeToLocal(path, content) {
    const file = this.vault.getAbstractFileByPath(path);
    if (file && file instanceof import_obsidian5.TFile) {
      await this.vault.modifyBinary(file, (0, import_obsidian5.base64ToArrayBuffer)(content));
      return { path, status: "changed" };
    } else if (!file) {
      this.ensureFolderExists(path);
      await this.vault.createBinary(path, (0, import_obsidian5.base64ToArrayBuffer)(content));
  ...
  async createCopyInDir(path, copyDir = "_fit") {
    ...
    if (file && file instanceof import_obsidian5.TFile) {
      ...
      if (copyFile && copyFile instanceof import_obsidian5.TFile) {
...
var FitPlugin = class extends import_obsidian6.Plugin {
  ...
  initializeFitNotice(addClasses = ["loading"], initialMessage) {
    const notice = new import_obsidian6.Notice(initialMessage && initialMessage.length > 0 ? initialMessage : " ", 0);
    notice.noticeEl.addClass("fit-notice");
    addClasses.map((cls) => notice.noticeEl.addClass(cls));
    return notice;
  }
  removeFitNotice(notice, finalClass, duration = 5e3) {
