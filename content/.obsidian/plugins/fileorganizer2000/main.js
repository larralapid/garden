...
var AbstractChatCompletionRunner = class {
  ...
  _emit(event, ...args) {
    ...
    if (event === "abort") {
      const error = args[0];
      if (!__classPrivateFieldGet7(this, _AbstractChatCompletionRunner_catchingPromiseCreated, "f") && !(listeners == null ? void 0 : listeners.length)) {
...
_AbstractChatCompletionRunner_connectedPromise = /* @__PURE__ */ new WeakMap(), _AbstractChatCompletionRunner_resolveConnectedPromise = /* @__PURE__ */ new WeakMap(), _AbstractChatCompletionRunner_rejectConnectedPromise = /* @__PURE__ */ new WeakMap(), _AbstractChatCompletionRunner_endPromise = /* @__PURE__ */ new WeakMap(), _AbstractChatCompletionRunner_resolveEndPromise = /* @__PURE__ */ new WeakMap(), _AbstractChatCompletionRunner_rejectEndPromise = /* @__PURE__ */ new WeakMap(), _AbstractChatCompletionRunner_listeners = /* @__PURE__ */ new WeakMap(), _AbstractChatCompletionRunner_ended = /* @__PURE__ */ new WeakMap(), _AbstractChatCompletionRunner_errored = /* @__PURE__ */ new WeakMap(), _AbstractChatCompletionRunner_aborted = /* @__PURE__ */ new WeakMap(), _AbstractChatCompletionRunner_catchingPromiseCreated = /* @__PURE__ */ new WeakMap(), _AbstractChatCompletionRunner_handleError = /* @__PURE__ */ new WeakMap(), _AbstractChatCompletionRunner_instances = /* @__PURE__ */ new WeakSet(), _AbstractChatCompletionRunner_getFinalContent = function _AbstractChatCompletionRunner_getFinalContent2() {
  return __classPrivateFieldGet7(this, _AbstractChatCompletionRunner_instances, "m", _AbstractChatCompletionRunner_getFinalMessage).call(this).content;
...
// src/modules/audio.ts
async function useAudio(filePath, apiKey) {
  const openai = new openai_default({ apiKey, dangerouslyAllowBrowser: true });
  const transcription = await openai.audio.transcriptions.create({
    file: import_fs2.default.createReadStream(filePath),
    model: "whisper-1"
  });
...
async function useText(content, systemPrompt = "", apiKey) {
  const data = {
    model: "gpt-4-1106-preview",
    temperature: 0,
    messages: [
...
var FileOrganizerSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian3.Setting(containerEl).setName("OpenAI API key").setDesc("Enter your API Key here").addText(
      (text) => text.setPlaceholder("Enter your API Key").setValue(this.plugin.settings.API_KEY).onChange(async (value) => {
        this.plugin.settings.API_KEY = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian3.Setting(containerEl).setName("Inbox Folder").setDesc("Choose which folder to automatically organize files from").addText(
    ...
    new import_obsidian3.Setting(containerEl).setName("Features").setHeading();
    new import_obsidian3.Setting(containerEl).setName("FileOrganizer logs").setDesc(
    ...
    new import_obsidian3.Setting(containerEl).setName("Similar tags").setDesc("Append similar tags to the processed file.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.useSimilarTags).onChange(async (value) => {
        this.plugin.settings.useSimilarTags = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian3.Setting(containerEl).setName("Folder config").setHeading();
    new import_obsidian3.Setting(containerEl).setName("Attachments folder").setDesc(
    ...
    new import_obsidian3.Setting(containerEl).setName("File Organizer log folder").setDesc("Choose a folder for Organization Logs e.g. Ava/Logs.").addText(
    ...
    new import_obsidian3.Setting(containerEl).setName("Output folder path").setDesc(
    ...
    new import_obsidian3.Setting(containerEl).setName("Experimental features").setHeading();
    new import_obsidian3.Setting(containerEl).setName("Custom vision prompt").setDesc("Enter your custom prompt for vision processing here").addText(
    ...
    new import_obsidian3.Setting(containerEl).setName("Experimental: Describe workfow (contact for access)").setDesc(
    ...
    new import_obsidian3.Setting(containerEl).setName("Experimental: Full Auto Org (contact for access)").setDesc("Let file Organizer work fully automatically.").setDisabled(true);
  }
};
...
var validImageExtensions = ["png", "jpg", "jpeg", "gif", "svg"];
...
var isValidExtension = (extension) => {
  if (!validExtensions.includes(extension)) {
    new import_obsidian4.Notice("Sorry, FileOrganizer does not support this file type.");
    return false;
  }
  return true;
};
var FileOrganizer = class extends import_obsidian4.Plugin {
  // all files in inbox will go through this function
  async processFileV2(file) {
    new import_obsidian4.Notice(`Looking at ${file.basename}`, 3e3);
    this.validateAPIKey();
    if (!file.extension)
      return;
  ...
  async handleMediaFile(file, content) {
  ...
  async handleNonMediaFile(file, content) {
    const humanReadableFileName = await this.generateNameFromContent(
      content
    );
    await this.moveAndTagContent(file, content, humanReadableFileName);
  }
  async moveAndTagContent(file, content, newFileName) {
  ...
  async createFileFromContent(content) {
    ...
    try {
      name = await name_default(content, this.settings.API_KEY);
    } catch (error) {
      console.error("Error processing file:", error.status);
      new import_obsidian4.Notice("Could not set a human readable name.");
    }
  ...
  async moveContent(file, humanReadableFileName, destinationFolder = "") {
    new import_obsidian4.Notice(`Moving file to ${destinationFolder}`, 3e3);
  ...
  async generateNameFromContent(content) {
    new import_obsidian4.Notice(`Generating name for ${content.substring(0, 20)}...`, 3e3);
    const name = await name_default(content, this.settings.API_KEY);
    const safeName = formatToSafeName(name);
    new import_obsidian4.Notice(`Generated name: ${safeName}`, 3e3);
    return safeName;
  }
  async generateTranscriptFromAudio(file) {
    new import_obsidian4.Notice(`Generating transcription for ${file.basename}`, 3e3);
    const filePath = file.vault.adapter.basePath + "/" + file.path;
    const transcribedText = await audio_default(filePath, this.settings.API_KEY);
  ...
  async generateImageAnnotation(file, customPrompt) {
    new import_obsidian4.Notice(`Generating annotation for ${file.basename}`, 3e3);
    const arrayBuffer = await this.app.vault.readBinary(file);
    const fileContent = Buffer.from(arrayBuffer);
    const encodedImage = fileContent.toString("base64");
    logMessage(`Encoded: ${encodedImage.substring(0, 20)}...`);
    const processedContent = await vision_default(
      encodedImage,
      this.settings.API_KEY,
      customPrompt
    );
  ...
  async getSimilarTags(content, fileName) {
    ...
    return mostSimilarTags.split(",").map((tag) => tag.trim()).filter((tag) => !content.includes(tag));
  }
  isTFolder(file) {
  ...
  async getAIClassifiedFolder(content, file) {
    ...
    let uniqueFolders = this.getAllFolders().filter((folder) => {
      var _a2;
      return folder !== ((_a2 = file.parent) == null ? void 0 : _a2.path);
    ...
    new import_obsidian4.Notice(`Most similar folder: ${mostSimilarFolder}`, 3e3);
...
${similarTags.join(" ")}`);
      new import_obsidian4.Notice(`Added similar tags to ${file.basename}`, 3e3);
      return;
    }
    new import_obsidian4.Notice(`No similar tags found`, 3e3);
  }
  ...
  async appendToCustomLogFile(contentToAppend, action = "") {
    ...
    const now = new Date();
    const formattedDate = (0, import_obsidian4.moment)(now).format("YYYY-MM-DD");
...
${content}`);
      ...
      new import_obsidian4.Notice("Error processing incoming file.");
    }
  }
  // native
  async onload() {
